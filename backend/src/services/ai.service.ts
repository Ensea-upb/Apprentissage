...
// Avoids repeated identical AI calls. The AI is only invoked when the requested
// content is not yet in cache (or has expired). Resets on process restart.

interface AiCacheEntry { value: unknown; expiresAt: number }
const _aiCache = new Map<string, AiCacheEntry>();

function getFromCache<T>(key: string): T | null {
  const entry = _aiCache.get(key);
  if (!entry) return null;
  if (Date.now() > entry.expiresAt) { _aiCache.delete(key); return null; }
  return entry.value as T;
}

function putInCache(key: string, value: unknown, ttlMs: number): void {
...
export async function generateVariant(params: {
  ...
  const originalId = params.originalQuestion.id as string;

  // Check DB for an existing variant — avoids AI call if one was already generated
  try {
    const existing = await prisma.question.findFirst({
      where: { variantOf: originalId, isStatic: false },
      orderBy: { createdAt: 'desc' },
    });
    if (existing) {
      console.info(`[AI] Reusing cached variant for ${originalId}`);
      return existing.content as Record<string, unknown>;
    }
  } catch {
    // DB check failed — proceed to generate
  }
...
Ne change pas le type, ne simplifie pas le concept, ne réduis pas la difficulté de plus d'un niveau.`;
  ...
  try {
    ...
    const result = {
      ...variant,
      id: `variant_${originalId}_${Date.now()}`,
      isStatic: false,
      variantOf: originalId,
    };

    // Persist to DB so future calls reuse this variant without calling AI again
    try {
      const original = await prisma.question.findUnique({
        where: { id: originalId },
        select: { phase: true, difficulty: true },
      });
      await prisma.question.create({
        data: {
          id: result.id as string,
          conceptId: params.conceptId,
          phase: original?.phase ?? 1,
          type: (variant.type as string) || (params.originalQuestion.type as string) || 'mcq',
          difficulty: (variant.difficulty as number) || (original?.difficulty ?? 3),
          content: result,
          isStatic: false,
          variantOf: originalId,
        },
      ...
      // Non-critical — variant returned even if DB save fails
    }
...
export async function generateDocumentation(params: {
  ...
  // Documentation for a given concept+errorType is deterministic — cache 30 days
  const cacheKey = `doc:${params.conceptId}:${params.errorType}`;
  const cached = getFromCache<Documentation>(cacheKey);
...
Structure obligatoire — réponds en JSON :
{
  ...
  try {
    ...
    const docs = JSON.parse(extractJSON(text)) as Documentation;
    putInCache(cacheKey, docs, 30 * 24 * 60 * 60 * 1000); // 30 days
    return docs;
  } catch (err) {
...
export async function getDailyInsight(recentConcepts: string[]): Promise<DailyInsight> {
  // One insight per calendar day (UTC) — shared across all users.
  // Generated once by the startup pre-warm, served from cache for all subsequent requests.
  const todayKey = `insight:${new Date().toISOString().slice(0, 10)}`;
  const cached = getFromCache<DailyInsight>(todayKey);
  if (cached) return cached;
...
Réponds en JSON :
{
  ...
  try {
    const text = await callClaude(prompt, { max_tokens: 512 });
    const insight = JSON.parse(extractJSON(text)) as DailyInsight;
    // Cache until next UTC midnight
    const nextMidnight = new Date();
    nextMidnight.setUTCDate(nextMidnight.getUTCDate() + 1);
    nextMidnight.setUTCHours(0, 0, 0, 0);
    putInCache(todayKey, insight, nextMidnight.getTime() - Date.now());
    return insight;
  } catch {
    return {
